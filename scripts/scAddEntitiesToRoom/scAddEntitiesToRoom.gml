...
for (_i = 0; _i < _numberOfEntitiesToPlace; _i++) {
	...
	if !scCheckIfEntityOccupiesTile(_placementX, _placementY) && !scCheckIfTileBlocksMovement(_placementX, _placementY) {
		//random number to determine what entitiy to generate from possible options 
		//***need to make entity choosing dynamic
		if irandom(100) < 80 {
			scCreateEntity(_placementX, _placementY, entityNumber.orc, true, scFighter, scBasicMonster);
			
		} else {
			scCreateEntity(_placementX, _placementY, entityNumber.troll, true, scFighter, scBasicMonster);	
		}
	}
}
